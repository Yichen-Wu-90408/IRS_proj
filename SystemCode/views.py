...
def get_conn():
    conn = pymysql.connect(host='127.0.0.1', port=3306, user='root', passwd='', db='MovieData', charset='utf8')
    return conn
...
if __name__ == '__main__':
    ratingfile3 = os.path.join('static', 'users_resulttable.csv')  # 一共671个用户

    usercf = UserBasedCF()
    userId = '1'
    # usercf.initial_dataset(ratingfile1)
    usercf.generate_dataset(ratingfile3)
    usercf.calc_user_sim()
    # usercf.evaluate()
